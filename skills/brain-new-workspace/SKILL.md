...
Bootstraps a new workspace inside the brain repo with the standard folder structure (`archive/`, `blockers/`, `decisions/`, `features/`, `feedback/`, `incidents/`, `knowledge/`, `phases/`, `sessions/`) and a project-type entity stub in `graph/entities/`.
...
4. **Sync state** — `git -C <brain> pull --rebase origin main` so the new workspace doesn't collide with one a teammate just added. Verify `git -C <brain> branch --show-current` is `main`.
...
1. **Workspace name** — kebab-case, short (e.g. `acme-app`, `acme-web`, `research`)
2. **Title** — human-readable name (e.g. "Acme App (mobile client)")
...
```bash
mkdir -p workspaces/{name}/{archive,blockers,decisions,features,feedback,incidents,knowledge,phases,sessions}
```
...
- `REPO_KNOWLEDGE.md` is auto-generated by CI (`knowledge-update.yml`) — never edit it; the new workspace's entity and folders are the canonical source it derives from.
